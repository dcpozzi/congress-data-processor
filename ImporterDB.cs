...
public class ImportData
{
    ...
    public static void Execute(JObject jsonData)
    {
        ...
        HashSet<String> updatedDeputados = new HashSet<String>();
        ...
        using (var conn = new NpgsqlConnection(ConnectionString))
        {
            ...
            {
                ClearDataBase(conn);
                ImportData.ImportDeputados(conn);
                Dictionary<String, int> deputadosAPI = GetAllDeputadosAPI(conn);

                try
                {
                    foreach (var item in jsonData["dados"])
                    {
                        idDocumento = item["idDocumento"].ToObject<int>();
                        ...
                        if (deputadosAPI.ContainsKey(nomeDeputado))
                        {
                            ...
                            {
                                if (UpdateDeputados(conn, idDeputado, idDeputadoArq))
                                {
                        ...
                        {
                            idDeputado = InsertDeputado(conn, nomeDeputado, idDeputadoArq);
                        ...
                        if (counter % 100 == 0)
                        {
                            Console.WriteLine($"Registers: {counter}");
                        }
                    }
    ...
    private static int InsertDeputado(NpgsqlConnection conn, string nomeDeputado, int idDeputadoArq)
    {
    ...
    private static bool UpdateDeputados(NpgsqlConnection conn, int idDeputado, int idDeputadoArq)
    {
        string updateDeputado = @"
            update deputados
    ...
    public static void ImportDeputados(NpgsqlConnection conn)
    {
    ...
    private static Dictionary<string, int> GetAllDeputadosAPI(NpgsqlConnection conn)
    {
        string query = @"
        SELECT id, nome FROM deputados";

        var deputados = new Dictionary<string, int>();
    ...
    private static void ClearDataBase(NpgsqlConnection conn)
    {
        string clearQuery = @"
            delete from gastos;
